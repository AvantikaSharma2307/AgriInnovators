...
const DiseasePredictionPage = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    formData.append('text', values.text); // Append text to form data

    try {
      // Replace with your actual API endpoint for disease prediction
      const response = await axios.post('http://localhost:5000/api/disease-prediction', formData, {
        headers: {
  ...
  return (
    <div className='bg-lime-200 h-full'>
      <div className="max-w-4xl mx-auto p-6">
            ...
            <Form className="space-y-6">
              ...
              <div>
                <label htmlFor="text" className="block text-lg font-medium text-gray-700">
                  Enter Name of Crop whose image is uploaded (e.g., "rice" or another for prediction)
                </label>
                <Field
                  id="text"
                  name="text"
                  type="text"
                  className="w-full p-3 mt-2 bg-transparent border border-gray-900 border-2  rounded-md"
                />
        ...
        {predictionResult && (
          <div className="mt-6 p-4 bg-green-500 text-center rounded-md">
